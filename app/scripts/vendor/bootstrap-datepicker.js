...
!(function ($) {
    ...
    function DatePicker(element, options) {
        ...
        var compiledOptions = $.extend({}, $.fn.datepicker.defaults, options);

        if ((!!compiledOptions.parse) ||
            (!!compiledOptions.format) ||
            !this.detectNative()) {

            $.extend(this, compiledOptions);
...
*/
            ...
            this.$picker = $('<div>')
                ...
                .addClass('datepicker number' + this.calendarNum)
                .append($nav, $calendar)
                .insertAfter(this.$el);
        ...
        nav: function (c, months) {
            var $subnav = $('<div>' +
                            ...
                            '</div>').addClass(c);
            $('.prev', $subnav)
                .click($.proxy(function () { this.ahead(-months, 0); }, this));
            $('.next', $subnav)
                .click($.proxy(function () { this.ahead(months, 0); }, this));
            return $subnav;
        ...
        show: function (e) {
            ...
            $.each(all, function (i, $picker) {

                left = left * (i + 1);
                console.log('showing calendar ' + i + ', top: ' + top + ', left: ' + left);
        ...
        parse: function (string) {
            ...
            //parse british date format into a Date.
            var m;
            if ((m = string.match(/^(\d{2,2})\/(\d{2,2})\/(\d{4,4})$/))) {
                return new Date(m[3], m[2] - 1, m[1]); //not sure about this line
            } else {
                return null;
            }
        },
        ...
        format: function (date) {
            // Format a Date into a british string.
            var month = (date.getMonth() + 1).toString(),
                dom = date.getDate().toString();

            //pad
            month = ((month.length === 1) ? '0' : '') + month;
            dom   = ((dom.length === 1) ? '0' : '')  + dom;
    ...
    $.fn.datepicker = function (options) {
        return this.each(function () {
            new DatePicker(this, $.extend({}, {'calendarNum': 1}, options));
            new DatePicker(this, $.extend({}, {'calendarNum': 2}, options));
        });
    };
